...
exports.listCategories = asyncHandler(async (req, res) => {
  ...
  success(
    res,
    await categoryService.listCategories({ includeInactive })
  );
});
...
exports.myDocuments = asyncHandler(async (req, res) => {
  success(
    res,
    await documentService.listDocuments(req.query, {
      mine: true,
      userId: req.user.id,
    })
  );
...
exports.getDocument = asyncHandler(async (req, res) => {
  success(
    res,
    await documentService.getDocument(req.params.id)
  );
});
...
/**
 * Download document from Supabase private storage.
 *
 * A temporary signed URL is generated by the backend.
 * The actual Supabase bucket remains private.
 */
exports.downloadDocument = asyncHandler(async (req, res) => {
  ...
  if (doc.storageProvider !== "supabase") {
    throw new ApiError(
      400,
      "This document is not stored in Supabase"
    );
  }
...
exports.adminDashboard = asyncHandler(async (req, res) => {
  success(
    res,
    await dashboardService.adminStats()
  );
});
...
exports.userDashboard = asyncHandler(async (req, res) => {
  success(
    res,
    await dashboardService.userStats(req.user.id)
  );
});
...
exports.auditLogs = asyncHandler(async (req, res) => {
  success(
    res,
    await dashboardService.listAuditLogs(req.query)
  );
});
